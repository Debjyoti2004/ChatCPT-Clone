...
export function ChatHeader({
  ...
  return (
    <div
      className={`flex items-center justify-between p-3 sm:p-4 border-b border-white/10 bg-gradient-to-r from-[#212121] to-[#252525] backdrop-blur-xl transition-all duration-300 shadow-lg ${
        isCollapsed ? "ml-0" : ""
      }`}
    >
      ...
      <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
        {showMemoryIndicator && (
          <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm text-green-400 animate-pulse bg-green-400/10 px-2 py-1 rounded-full border border-green-400/20">
            <Brain className="w-3 h-3 sm:w-4 sm:h-4 animate-glow" />
            <span className="hidden sm:inline">Memory updated</span>
          </div>
        )}

        {uploadedFiles.length > 0 && (
          <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm text-blue-400 bg-blue-400/10 px-2 py-1 rounded-full border border-blue-400/20">
